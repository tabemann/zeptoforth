...
zeptoIP is an IP stack for zeptoforth, which at the present is layered on top of a CYW43439 driver for the Raspberry Pi Pico W. This is currently in an beta state, and is being made available for testing prior to becoming part of the zeptoforth master branch. Note that it cannot be used on top of any currently existing build of zeptoforth but rather requires the latest code in the `devel` branch.
