...
Optional support for turtle graphics is in `extra/rp_common/turtle*.fs`. It is designed for compatibility with Thurtle, which is based on WAForth and runs in web browsers, and Forthtoise, which is on top of gforth and uses SDL. It allows changing the color of both the lines drawn and the turtle, but does not currently support changing the line thickness (even though that will likely change in the near future).

There are the following versions:

- `extra/rp_common/turtle.fs`: 16-bit graphics on the 160x80 ST7735S-based display on the Waveshare RP2040-LCD-0.96 (this can be readily modified for other ST7735S-based displays)
- `extra/rp_common/turtle_st7789v.fs`: 8-bit graphics on the 320x240 ST7789V-based display on the Tufty 2040 (this can be readily modified for other parallel ST7789V-based displays)
- `extra/rp_common/turtle_st7789v_spi.fs`: 8-bit graphics on SPI 320x240 ST7789V-based displays
- `extra/rp_common/turtle_picocalc.fs`: 8-bit graphics on PicoCalc's 320x320 display
